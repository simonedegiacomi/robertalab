...
public class ImageInvertFunction<V> extends Function<V> {
    ...
    private ImageInvertFunction(Expr<V> image, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(BlockTypeContainer.getByName("IMAGE_INVERT"), properties, comment, error);
     ...
     */
    public static <V> ImageInvertFunction<V> make(Expr<V> image, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new ImageInvertFunction<>(image, properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        ...
        return ImageInvertFunction.make(helper.convertPhraseToExpr(image), helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
