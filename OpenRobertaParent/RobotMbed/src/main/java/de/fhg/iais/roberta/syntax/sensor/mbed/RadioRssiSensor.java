...
public class RadioRssiSensor<V> extends Sensor<V> {
    ...
    private RadioRssiSensor(BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(BlockTypeContainer.getByName("RADIO_RSSI"), properties, comment, error);
        setReadOnly();
    }
     ...
     */
    public static <V> RadioRssiSensor<V> make(BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new RadioRssiSensor<V>(properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        return RadioRssiSensor.make(helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
