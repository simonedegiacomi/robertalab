...
public final class RandomEyesDuration<V> extends Action<V> {
    ...
    private RandomEyesDuration(Expr<V> duration, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(BlockTypeContainer.getByName("RANDOM_EYES_DURATION"), properties, comment, error);
     ...
     */
    private static <V> RandomEyesDuration<V> make(Expr<V> duration, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new RandomEyesDuration<V>(duration, properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        ...
        return RandomEyesDuration.make(helper.convertPhraseToExpr(duration), helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
