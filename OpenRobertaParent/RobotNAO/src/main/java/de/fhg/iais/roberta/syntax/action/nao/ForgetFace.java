...
public class ForgetFace<V> extends Action<V> {
    ...
    private ForgetFace(Expr<V> faceName, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(BlockTypeContainer.getByName("FORGET_FACE"), properties, comment, error);
     ...
     */
    private static <V> ForgetFace<V> make(Expr<V> faceName, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new ForgetFace<>(faceName, properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        ...
        return ForgetFace.make(helper.convertPhraseToExpr(msg), helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
