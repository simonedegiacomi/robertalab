...
public final class NaoMarkInformation<V> extends Sensor<V> {
    ...
    private NaoMarkInformation(Expr<V> naoMarkId, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(BlockTypeContainer.getByName("NAO_MARK_INFORMATION"), properties, comment, error);
     ...
     */
    static <V> NaoMarkInformation<V> make(Expr<V> naoMarkId, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new NaoMarkInformation<>(naoMarkId, properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        ...
        return NaoMarkInformation.make(helper.convertPhraseToExpr(naoMarkId), helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
