...
public class PlayFile<V> extends Action<V> {
    ...
    private PlayFile(Expr<V> msg, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(BlockTypeContainer.getByName("PLAY_FILE"), properties, comment, error);
     ...
     */
    private static <V> PlayFile<V> make(Expr<V> msg, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new PlayFile<>(msg, properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        ...
        return PlayFile.make(helper.convertPhraseToExpr(msg), helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
