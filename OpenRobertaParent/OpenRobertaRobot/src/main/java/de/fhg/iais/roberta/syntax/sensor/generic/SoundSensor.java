...
public class SoundSensor<V> extends ExternalSensor<V> {
    ...
    private SoundSensor(SensorMetaDataBean sensorMetaDataBean, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(sensorMetaDataBean, BlockTypeContainer.getByName("SOUND_SENSING"), properties, comment, error);
        setReadOnly();
    }
     ...
     */
    public static <V> SoundSensor<V> make(SensorMetaDataBean sensorMetaDataBean, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new SoundSensor<V>(sensorMetaDataBean, properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        ...
        return SoundSensor.make(sensorData, helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
}
