...
public final class BrushOff<V> extends Action<V> {
    ...
    private BrushOff(BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(BlockTypeContainer.getByName("BRUSH_OFF"), properties, comment, error);
        setReadOnly();
    }
     ...
     */
    private static <V> BrushOff<V> make(BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new BrushOff<V>(properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        return BrushOff.make(helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
