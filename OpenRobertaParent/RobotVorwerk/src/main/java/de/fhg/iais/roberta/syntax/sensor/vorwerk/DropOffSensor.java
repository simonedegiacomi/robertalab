...
public final class DropOffSensor<V> extends ExternalSensor<V> {
    private DropOffSensor(SensorMetaDataBean sensorMetaDataBean, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(sensorMetaDataBean, BlockTypeContainer.getByName("DROP_OFF_SENSOR"), properties, comment, error);
        setReadOnly();
    }
     ...
     */
    public static <V> DropOffSensor<V> make(SensorMetaDataBean sensorMetaDataBean, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new DropOffSensor<>(sensorMetaDataBean, properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        ...
        return DropOffSensor.make(sensorData, helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
