...
public final class BrushOn<V> extends Action<V> {
    ...
    private BrushOn(Expr<V> speed, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(BlockTypeContainer.getByName("BRUSH_ON"), properties, comment, error);
     ...
     */
    private static <V> BrushOn<V> make(Expr<V> speed, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new BrushOn<V>(speed, properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        ...
        return BrushOn.make(helper.convertPhraseToExpr(speed), helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
