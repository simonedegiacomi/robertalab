...
public class SendIRAction<V> extends Action<V> {
    ...
    private SendIRAction(Expr<V> code, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
     ...
     */
    private static <V> SendIRAction<V> make(Expr<V> code, BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new SendIRAction<>(code, properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        ...
        return SendIRAction.make(helper.convertPhraseToExpr(code), helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
