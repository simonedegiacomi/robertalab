...
public class CodePadSensor<V> extends Sensor<V> {
    ...
    private CodePadSensor(BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(BlockTypeContainer.getByName("BOB3_CODEPAD"), properties, comment, error);
        setReadOnly();
    }
     ...
     */
    public static <V> CodePadSensor<V> make(BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new CodePadSensor<V>(properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        return CodePadSensor.make(helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
