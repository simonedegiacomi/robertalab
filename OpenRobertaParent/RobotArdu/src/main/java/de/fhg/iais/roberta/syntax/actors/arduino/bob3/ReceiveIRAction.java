...
public class ReceiveIRAction<V> extends Action<V> {
    ...
    private ReceiveIRAction(BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        super(BlockTypeContainer.getByName("BOB3_RECVIR"), properties, comment, error);
        setReadOnly();
    }
     ...
     */
    private static <V> ReceiveIRAction<V> make(BlocklyBlockProperties properties, BlocklyComment comment, BlocklyError error) {
        return new ReceiveIRAction<>(properties, comment, error);
    }
    ...
    public static <V> Phrase<V> jaxbToAst(Block block, AbstractJaxb2Ast<V> helper) {
        return ReceiveIRAction.make(helper.extractBlockProperties(block), helper.extractComment(block), helper.extractError(block));
    }
